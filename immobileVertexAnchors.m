...
min_points = 4;
GLOBAL_DENSITY = total_vertices/cell_area;
...
disp(size(slow_trajs))
search_radius = 30;

% Quick merge
[merged_coords, merged_trajs] = FastMergeOverlappingAnchors(slow_anchors, slow_trajs, finalTrajmin5, search_radius, min_points, LOC_ACC, GLOBAL_DENSITY);

disp(size(merged_trajs))

% Merge overlapping anchors and finalize anchors
[anchor_coords, anchor_trajs] = SlowMergeOverlappingAnchors(merged_coords, merged_trajs, finalTrajmin5, search_radius, min_points, LOC_ACC, GLOBAL_DENSITY );