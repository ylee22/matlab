function [ flattened_trajs_spots, larger_anchor_coords, first_last_anchor_frames ] = test2( finalTraj, cluster_anchor_coords, cluster_trajs, immobile_coords, immobile_anchor_coords, immobile_spots, localization_acc )
...
for anchor_idx = 1:length(flattened_trajs_spots)
    ...
    elseif iscell(smaller_coords) && ~iscell(flattened_trajs_spots{anchor_idx}) && flattened_trajs_spots{anchor_idx}(1) == -1
        ...
        larger_anchor_coords(anchor_idx,:) = combinedandTrajAnchorRadiusandCenter(larger_coords, smaller_coords, flattened_trajs_spots{anchor_idx});
    end
end
    ...
    if ~isempty(flattened_trajs_spots{anchor})
        ...
        if ~isempty(final_overlaps)
            ...
            elseif iscell(smaller_coords) && ~iscell(flattened_trajs_spots{anchor}) && flattened_trajs_spots{anchor}(1) == -1
                ...
                larger_anchor_coords(anchor,:) = combinedandTrajAnchorRadiusandCenter(larger_coords, smaller_coords, flattened_trajs_spots{anchor});
            end
        end
    end
end
...
if iscell(trajs_and_spots1)
    ...
    elseif trajs_and_spots2(1) == -1
        combined_trajs_and_spots = {cat(2,trajs_and_spots1{1},trajs_and_spots2{1}(2:end)),trajs_and_spots1{2}};
...
function radius_and_coords = combinedandTrajAnchorRadiusandCenter(large_coords,small_coords,current_trajs_spots)
